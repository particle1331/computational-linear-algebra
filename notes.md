...
<br><br>
We prove the SVD exists by construction. Let $\text{rank }\bold A = r \leq \min(m, n)$. Observe that $\bold A^\top \bold A \in \mathbb R^{n\times n}$ is symmetric positive semidefinite. It follows that $\bold A^\top \bold A = \bold V \bold \Lambda \bold V^\top$ where $\bold V$ is an orthogonal matrix and $\bold \Sigma$ is a diagonal matrix of real eigenvalues of $\bold A^\top \bold A$ [[Theorem 8.3]](https://www.maa.org/sites/default/files/pdf/awards/Axler-Ford-1996.pdf) (self-adjoint = symmetric). Moreover, since $\bold A^\top \bold A$ is positive semidefinite, the entries of $\bold\Lambda$ are all nonnegative. Thus, we can write $\bold A^\top \bold A = \bold V \bold \Sigma^2 \bold V^\top$ where $\bold\Sigma^2 = \bold\Lambda$, i.e. $\sigma_i = \sqrt{\lambda_i}$ for $i=1,2, \ldots, n$. This is precisely the equation $\bold P = \bold V \bold \Sigma \bold V^\top = \sqrt{\bold A^\top \bold A}$ in the polar decomposition (4.51) of $\bold A$! It can be shown that $\text{rank }(\bold A ^\top \bold A) = \text{rank }\bold A = r.$ Thus, $\bold\Sigma$ has $r \leq \min(m, n)$ nonzero diagonal entries. We do two things: (1) arrange that the eigendecomposition of $\bold A^\top \bold A$ is such that $\sigma_1 \geq \sigma_2 \geq \ldots \sigma_r \geq 0$, and (2) trim $\bold\Sigma$ to be $m \times n$ as in the figure below. Note that if $\bold A$ doesn't have full rank then $\sigma_j = 0$ for some $1 \leq j \leq m$ which results in the hyperellipse collapsing to zero volume in $\mathbb R^m.$ Now that we are done with the domain of $\bold A$, we proceed with its codomain. <br><br> We know $\bold A \bold v_i$ for $i = 1, \ldots, n$ span the image of $\bold A.$ Keeping track of the dimension of the image of $\bold A$, we consider only the first $r$ eigenvectors $\bold v_1, \ldots, \bold v_r$. Define vectors $\bold u_i \in \mathbb R^m$ as the vectors such that $\bold A \bold v_i = \sigma_i \bold u_i$ for $i =1, 2, \ldots, r.$ Observe that the vectors $\bold u_i$ are orthonormal: 
  $$
  ...
  $$
  The vectors $\bold u_1, \ldots \bold u_r$ is a basis for the image of $\bold A$ in $\mathbb R^m.$ Let $\bold U = [\bold u_1, \ldots, \bold u_r] \in \mathbb R^{m \times r}$ We can extend this to an orthonormal basis of $\mathbb R^m$ by Gram-Schmidt so that $\bold U \in \mathbb R^{m \times m}$ (see fig. below). Finally, $\bold U \bold \Sigma = \bold A \bold V$ so that $\bold U \bold \Sigma \bold V^\top = \bold A$ and we're done! $\square$  
<br>
...
* **Singular vectors in the SVD.** Given the SVD we can write $\bold A = \sum_{i=1}^r \sigma_i \bold u_i \bold v_i^\top$ as a sum of rank one (!) terms. Recall that $\sigma_i \bold u_i = \bold A \bold v_i$. Writing $\bold A = \sum_{i=1}^r (\bold A \bold v_i) \bold v_i^\top$ is trivial given an ONB $\bold v_1, \ldots, \bold v_n$ of $\mathbb R^n.$ What is nontrivial in the SVD is that (1) an ONB always exists that is natural to $\bold A$, and (2) the corresponding images $\bold A \bold v_i$ are also orthogonal in $\mathbb R^m.$  <br>
  <p align="center">
...
* **SVD as diagonalization.** We can think of the SVD as a change of basis so that the $m \times n$ matrix $\bold A$ is diagonal. That is, we diagonalize $\bold A$ in terms of two orthonormal bases of singular vectors for $\mathbb R^n$ and $\mathbb R^m$ resulting in $\bold U^\top \bold A \bold V = \bold \Sigma.$ Recall that we recover the components of a vector in an ONB by performing projection, so we can replace inverses with transpose. Let us see this in action: $\bold U^\top \bold A \bold V [\bold x]_{\mathcal V} = \bold \Sigma [\bold x]_{\mathcal V}$ where the LHS is $\bold U^\top \bold A \bold V \bold V^\top \bold x = \bold U^\top (\bold A \bold x) = [\bold A \bold x]_{\mathcal U}$ so that $[\bold A \bold x]_{\mathcal U} = \bold \Sigma [\bold x]_{\mathcal V}.$ (This is all very simple and is essentially a restatement of $\bold A = \sum_{i=1}^r \sigma_i \bold u_i \bold v_i^\top$ where $\bold u_i$ and $\bold v_i$ are ONBs! But writing it in the language of change of basis matrices might be instructive when reading the literature and connecting to the idea of diagonalization.) Thus, the SVD is analogous to diagonalization for square matrices, but instead of eigenvalues, we diagonalize into a diagonal matrix of singular values. Here is a lucid account of the idea in [Chapter 10](https://www.mathworks.com/content/dam/mathworks/mathworks-dot-com/moler/eigs.pdf) of Moler's *Numerical Computing with MATLAB*: 
