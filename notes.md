...
We prove the SVD exists by construction. Let $\text{rank }\bold A = r \leq \min(m, n)$. Observe that $\bold A^\top \bold A \in \mathbb R^{n\times n}$ is symmetric positive semidefinite. It follows that $\bold A^\top \bold A = \bold V \bold \Lambda \bold V^\top$ where $\bold V$ is an orthogonal matrix and $\bold \Sigma$ is a diagonal matrix of real eigenvalues of $\bold A^\top \bold A$ [[Theorem 8.3]](https://www.maa.org/sites/default/files/pdf/awards/Axler-Ford-1996.pdf) (self-adjoint = symmetric). Moreover, since $\bold A^\top \bold A$ is positive semidefinite, the entries of $\bold\Lambda$ are all nonnegative. Thus, we can write $\bold A^\top \bold A = \bold V \bold \Sigma^2 \bold V^\top$ where $\bold\Sigma^2 = \bold\Lambda$, i.e. $\sigma_i = \sqrt{\lambda_i}$ for $i=1,2, \ldots, n$. This is precisely the equation $\bold P = \bold V \bold \Sigma \bold V^\top = \sqrt{\bold A^\top \bold A}$ in the polar decomposition (4.51) of $\bold A$! It can be shown that $\text{rank }(\bold A ^\top \bold A) = \text{rank }\bold A = r.$ Thus, $\bold\Sigma$ has $r \leq \min(m, n)$ nonzero diagonal entries. We do two things: (1) arrange that the eigendecomposition of $\bold A^\top \bold A$ is such that $\sigma_1 \geq \sigma_2 \geq \ldots \sigma_r \geq 0$, and (2) trim $\bold\Sigma$ to be $m \times n$ as in the figure below. Note that if $\bold A$ doesn't have full rank then $\sigma_j = 0$ for some $1 \leq j \leq m$ which results in the hyperellipse collapsing to zero volume in $\mathbb R^m.$ Now that we are done with the domain of $\bold A$, we proceed with its codomain. <br><br> We know $\bold A \bold v_i$ for $i = 1, \ldots, n$ span the image of $\bold A.$ Keeping track of the dimension of the image of $\bold A$, we consider only the first $r$ eigenvectors $\bold v_1, \ldots, \bold v_r$. Define unit vectors $\bold u_i \in \mathbb R^m$ as the vectors such that $\bold A \bold v_i = \sigma_i \bold u_i$ for $i =1, 2, \ldots, r.$ Note that $r \leq m$ quite nicely. Observe that the vectors $\bold u_i$ are orthonormal: 
  ...
  $$
  Let $\bold U = [\bold u_1, \ldots, \bold u_r] \in \mathbb R^{m \times n}$ We can extend this to an orthonormal basis of $\mathbb R^m$ by Gram-Schmidt so that $\bold U \in \mathbb R^{m \times m}$ (see fig. below). Finally, $\bold U \bold \Sigma = \bold A \bold V$ so that $\bold U \bold \Sigma \bold V^\top = \bold A$ and we're done! $\square$  
<br>
